...
const get = (pass: Map<string, unknown>, name: string) => pass.get(`babel-plugin-million/${name}`);
const set = (pass: Map<string, unknown>, name: string, v: unknown) =>
  pass.set(`babel-plugin-million/${name}`, v);
    ...
    const injectMetaPropertiesVisitor = {
      JSXOpeningElement(path, state) {
        ...
        if (isThisAllowed(path)) {
          attributes.push(
            t.jsxAttribute(t.jsxIdentifier('__self'), t.jsxExpressionContainer(t.thisExpression())),
          );
        }
        attributes.push(
          t.jsxAttribute(
